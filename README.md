...
The Art Museum [raised-park review](https://mke3d.vercel.app/museum-review.html?view=park) includes additional lawn-edge trees and low planting on the garage roof and western ground-level lawns. Planting follows mapped grass footprints, keeps the bridge landing open, and uses the roof elevation independently of terrain. Locations and sizes are interpreted from the supplied aerial references.

### Ride the Skyglider

Choose **Ride Skyglider** in the main city or Summerfest review to sit in an actual animated chair. The first-person camera follows the same cable route, slopes, and terminal turns as the visible chairs. Drag or use arrow keys to look around; **Exit Skyglider** or **Esc** returns to the previous map view. The ride uses the existing photo-based route and chair dimensions rather than a separate simulated flight.
