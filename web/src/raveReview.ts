...
import {RAVE_SITE as s} from './raveSite';
...
const model=buildRave(groundAt),centered=new THREE.Group();centered.position.set(-s.x,0,-s.z);centered.add(model);scene.add(centered);
...
const views:Record<string,View>={
 corner:{eye:[75,55,-120],target:[0,14,-6]},
 north:{eye:[0,23,-125],target:[0,14,-22]},
 entry:{eye:[0,7,-73],target:[0,7,-34]},
 east:{eye:[115,35,0],target:[0,14,0]},
 river:{eye:[-110,35,0],target:[0,14,0]},
 rear:{eye:[0,40,130],target:[0,14,15]},
 roof:{eye:[50,145,-65],target:[0,10,0]},
};
function view(key:string){camera.position.set(...views[key].eye).add(new THREE.Vector3(0,s.floor,0));controls.target.set(...views[key].target).add(new THREE.Vector3(0,s.floor,0));controls.update();}view('corner');
