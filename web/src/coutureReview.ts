...
import { COUTURE_SITE as site } from './coutureSite';
...
const model=buildCouture(()=>2.5);model.position.x=0;model.position.z=0;scene.add(model);
const ground=new THREE.Mesh(new THREE.PlaneGeometry(2000,2000),new THREE.MeshStandardMaterial({color:0xb8bfbc,roughness:1}));
ground.rotation.x=-Math.PI/2;ground.position.y=2.49;ground.receiveShadow=true;scene.add(ground);
// Reference track centerline is the existing mapped north-to-south hall alignment.
for(const offset of [-.7175,.7175]){
 const a=new THREE.Vector3(site.transit.north[0]-site.x+offset,site.transit.northRailY-.025,site.transit.north[1]-site.z);
 const b=new THREE.Vector3(site.transit.south[0]-site.x+offset,site.transit.southRailY-.025,site.transit.south[1]-site.z);
 const d=b.clone().sub(a),g=new THREE.BoxGeometry(.055,.05,d.length());
 g.applyQuaternion(new THREE.Quaternion().setFromUnitVectors(new THREE.Vector3(0,0,1),d.normalize()));g.translate(...a.add(b).multiplyScalar(.5).toArray());
 scene.add(new THREE.Mesh(g,new THREE.MeshStandardMaterial({color:0x687375,metalness:.6,roughness:.4})));
}
const views:Record<string,{eye:[number,number,number],target:[number,number,number]}>= {
 east:{eye:[170,100,-180],target:[-12,80,0]},west:{eye:[-195,100,145],target:[-12,80,0]},
 crown:{eye:[-65,192,50],target:[0,146,0]},
 hall:{eye:[-22,7,-58],target:[-39,7,35]},
};
